...
Analyzes your AWS infrastructure for cost savings opportunities using Claude AI. Manage multiple AWS accounts through a web UI, run audits across eight service types (or all at once with Full Audit), and get actionable recommendations with estimated savings. View cost trends over time, get step-by-step implementation instructions, and execute fixes directly from the UI. Mark recommendations as fixed or incorrect and those resolutions carry forward to future audits automatically.
...
### OpenSearch
- **Idle domains**: Near-zero search and indexing rate — candidates for deletion
- **Underutilized domains**: Very low CPU and search rate with right-sizing suggestions
- **Oversized storage**: Free storage exceeds 75% of total — could downsize volume
- **Old engine versions**: Outdated OpenSearch or Elasticsearch engine versions
- **No Reserved Instances**: Domains not covered by RIs that could benefit from commitments
- **Single-AZ**: Multi-node clusters without Multi-AZ, risking availability
- **Unnecessary dedicated masters**: Small clusters paying for dedicated master nodes they don't need
- **GP2 → GP3 migration**: EBS GP2 volumes that can upgrade to GP3 for better price/performance
- **Warm/cold tier candidates**: High-storage, low-search domains suited for UltraWarm or cold storage
- **Graviton migration**: Intel/AMD instances that could run on cheaper Graviton (~20% savings)
- **Overprovisioned IOPS**: io1/io2 volumes with low IOPS utilization
- **Node consolidation**: Too many nodes for the workload — could consolidate
- **JVM memory pressure**: High JVM heap usage indicating OOM/GC risk
- **Cluster health**: Clusters not consistently in green health status

### Full Audit
Runs all eight service audits in parallel, then consolidates the results:

1. **Parallel execution** — Launches EC2, RDS, S3, NAT, Lambda, DynamoDB, ELB, and OpenSearch audits simultaneously with per-service progress tracking in the UI
...
Interactive cost trends dashboard powered by AWS Cost Explorer data:

- **Cost trend charts** — Stacked area charts showing monthly or daily costs broken down by AWS service
- **Filters** — Account selector, date range (3/6/12 months), granularity (monthly/daily), and service filter chips for the top 12 services
- **Summary cards** — Latest period cost, multi-month total, top spending service, and month-over-month cost delta
- **Service breakdown table** — Aggregated costs by service with percentages and visual progress bars

## Solutions

Click any recommendation to get implementation instructions generated by Claude:

- **Console instructions** — Step-by-step guide through the AWS Management Console with exact menu paths, button names, and settings using your actual resource IDs and regions
- **CLI commands** — Ready-to-run AWS CLI commands with all necessary flags and actual resource identifiers (no placeholders)
- **In-app execution** — Edit and execute CLI commands directly from the UI using your stored account credentials, with live output display
...
Toggle between light and dark themes via the moon/sun icon in the top-right navbar. Your preference is saved to localStorage and persists across sessions.
  ...
  "Statement": [
    {
      ...
      "Action": [
        ...
        "opensearch:ListTags",
        "opensearch:DescribeReservedInstances"
      ],
      "Resource": "*"
    }
  ]
...
The backend includes a comprehensive test suite with 139 tests across 7 files using Vitest:
...
Covers all routes (accounts, audits, reports, solutions), services (recommendation validator, full audit dedup/subsumption), and the crypto module (AES-256-GCM encrypt/decrypt).

## Architecture

Each of the eight audit types follows a three-layer pattern:
...
The **Full Audit** runner acts as a meta-orchestrator — it launches all eight service runners in parallel, waits for completion, then runs a multi-pass deduplication and cross-service synthesis pipeline.
