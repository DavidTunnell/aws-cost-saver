...
export async function runAudit(accountId: number, auditId: number) {
  try {
    ...
    const data = await collectAccountData(
      ...
      (msg) => console.log(`[Audit ${auditId}] ${msg}`)
    );

    // Analyze with Claude
    console.log(`[Audit ${auditId}] Analyzing with Claude...`);
    const recommendations = await analyzeWithClaude(data);
    console.log(
      `[Audit ${auditId}] Got ${recommendations.length} recommendations`
    );
    ...
    console.log(
      `[Audit ${auditId}] Completed. Total potential savings: $${totalSavings.toFixed(2)}/mo`
    );
  } catch (err: any) {
